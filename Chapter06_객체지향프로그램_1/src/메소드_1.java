 ...
 *            ------- 매개변수 => 메소드 안에서만 사용하는 변수 (지역변수와 동일)
 *                    |
 *                    사용자로부터 요청값을 받은 경우
 * => 소스는 동일
 ...
 */
public class 메소드_1 {
	static void gugudan(int dan) // 매개변수 O, 리턴 X
	{
	...
	static void process() // 매개변수 X, 리턴 X
	{
		System.out.println("2. process():진입");
		Scanner scan=new Scanner(System.in);
		System.out.println("2~9 사이의 정수 입력:");
		int dan=scan.nextInt();
		gugudan(dan); // 입력값을 gugudan 메소드에 전달
		System.out.println("6. process():종료");
	}
	
	// 시작점 => 프로그램은 main이 없는 경우에는 실행 X
	// 모든 프로그램은 main이 한 개 이상 존재
