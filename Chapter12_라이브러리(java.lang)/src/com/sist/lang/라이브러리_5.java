 ...
 *
 *		주요 메소드 (기능) => 오버라이딩
 *			= finalized() : 객체 소멸 시에 자동 호출
 *			   소멸자 함수 => 재정의하지 않는다 
 *			= ***toString() : 객체를 문자열로 변경
 *             => 오버라이딩이 가장 많다
 *             => 재정의 없이 호출하면 객체의 주소값을 리턴한다
 *             => 생략 가능
 *             => 오버라이딩 => 실제 객체의 변수값 출력 
 *          = ***clone() : 복제
 *             => 실제 가지고 있는 변수값을 새로운 메모리에 저장
 *             => 스프링 : prototype
 *                ---- singleton
 *          = equals() : 객체의 주소값 비교
 *             => 많이 사용하지 않는다
 *             => String에서는 문자열만 비교
 *          = hashCode() : 메모리 주소를 기반해서 정수값으로 표현
 *             => 모든 객체는 hashCode()를 가지고 있다
