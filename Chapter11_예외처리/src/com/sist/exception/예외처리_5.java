 ...
 * 	예) 버튼 클릭, 마우스 클릭... 이벤트는 인터페이스로 되어있다
 * 
 * 	1) 버튼 / 메뉴 / 텍스트 필드 엔터
 * 	   ------------------------ ActionListener
 * 		=> actionPerformed()
...
public class 예외처리_5 extends JFrame implements ActionListener{
	...
	public 예외처리_5()
	{
		...
		tf=new JTextField(10); // NumberFormatException
		tf.setEnabled(false);
		// 웹 / 윈도우 => Only String
		// --------- 크롤링 (String)
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(e.getSource()==tf) // 엔터
		{
			// 입력값 읽기
			String temp=tf.getText();
			if(temp.trim().length()!=3)
			{
				setMessage("세 자리 정수만 입력 가능합니다");
				return; // 메소드 종료
			}
