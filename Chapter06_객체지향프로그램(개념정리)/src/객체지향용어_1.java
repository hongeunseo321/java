 ...
 * ------------
 * => 객체 생명 주기
 * 	class => 설계 => 메모리에 저장 => 필요 시에 사용 => 메모리 소멸
 *  ------------  |                    |            |
 ...
 *	}
 *
 *	변수 => 저장 위치, 선언 위취, 생명 주기
 *	1) 지역 변수
 *		선언 위치 : 메소드, 생성자, 블록 내부에 선언
 *  	** 반드시 초기값을 부여해서 사용
 *  	저장 위치 : 스택 => 메모리 자체에서 관리
 *		생명 주기 : {} 안에서만 사용 가능 => {} 벗어나면 자동 회수
 *		class A
 *		{인스턴스 선언 위치
 *			void display()
 *			{
 ...
 *
 *	2) 인스턴스 변수
 *		클래스 내부 블록에 선언 => 클래스 위나 아래에 선언이 가능하다
 *		객체 생성 시 메모리에 저장 => new를 사용할 때 메모리에 저장
 *                              --- 새로운 메모리가 생성
 *      초기값을 지정하지 않는 경우 => 자동으로 기본값이 설정된다
 ...
 *	3) 정적 변수
 *		모든 객체가 공유하는 변수
 *		*** 구분 (따로 저장되는 변수, 한 개를 갖고 사용)
 *		*** 벤치마킹 
 *		컴파일 시에 자동 생성
 *		프로그램 종료 시까지 메모리 유지
 *
 *	핵심)
 *		지역 변수 : 메소드 안에서 사용, 초기값을 반드시 부여
 *		인스턴스 변수 : 객체마다 따로 메모리가 생성, 기본값이 있다
 *		---------- 객체.변수
 *		정적 변수 : 클래스 변수, 공유 변수
 *                -------- 클래스명.변수
 *                모든 객체가 데이터를 공유
 ...
 *                
 *	** 초기화 여부
 *		지역 변수 : 필수, 직접 대입, 연산 결과, 기본값은 없다
 *		인스턴스 변수 : 선택, 선언 시 대입, 생성자에서 대입, 기본값이 있다
 *		정적 변수 : 선택, 선언 시 대입, 초기화 블록, 기본값이 있다
 *
 *	** 초기화 방법
 *		1) 선언과 동시에 초기화 : 명시적 초기화 => 고정 (한 개의 값)
 *		2) 생성자를 통해서 초기화 => 여러 개 값을 지정
 ...
 *		|변수에 대한 초기값 => 매개변수
 *
 *		** 반드시 모든 클래스는 생성자가 한 개 이상이 필요하다
 *			=> 생성자가 없는 경우에는 컴파일러가 자동으로 기본 생성자 추가
 *				(기본 생성자 : 매개변수가 없는 생성자)
 *
 *	=> 생성자의 종류
 *		기본 생성자 (default 생성자) => 자동 추가가 될 수 있다
 *		 => 시작과 동시에 처리해야 되는 기능 설정
 *		매개변수가 있는 생성자
