 ...
 *                         데이터베이스 연동
 * 		3) 종단 메소드 => final
 * 			상속에서 오버라이딩이 안 되는 메소드
 * 		4) 선언만 하는 메소드 => abstract
 * 			인터페이스
 * 			예) 구현이 안 되는 버튼 클릭, 마우스 클릭...
 * 
 * 	----------------------------------------
 ...
 *  
 *  	protected => 같은 폴더까지만 접근이 가능
 *                   다른 폴더인 경우에 상속이 있어야 접근이 가능
 ...
 *  	[접근지정어] 리턴형 메소드명(매개변수...)
 *  	{
 *  		return 값 ==> void를 생략하면 자동으로 컴파일러가 된다 
 *  	}      |
 *          메소드 종료
 *          1. import java.lang.* => String, System, Math...
 *          2. 기본 생성자
 *          3. 메소드 void일 경우 => return
 *          4. extends Object
 *          	=> 모든 클래스는 Object 상속
 *          5. try~catch
