...
public class JoinForm extends JFrame{
	...
	public JoinForm()
	{
		for(int i=0;i<las.length;i++)
		{
			...
			las[i].setBounds(30, 115+(i*35), 65, 30);
			add(las[i]);
		}
		...
		// ID
		id=new JTextField();
		id.setBounds(100, 115, 150, 30);
		add(id);
		
		// ID 중복체크
		idCheck=new JButton("중복체크");
		idCheck.setBounds(260, 115, 100, 30);
		add(idCheck);
		
		// 비밀번호
		pwd=new JPasswordField();
		pwd.setBounds(100, 150, 150, 30);
		add(pwd);
		
		// 이름
		name=new JTextField();
		name.setBounds(100, 185, 150, 30);
		add(name);
		
		// 성별 라디오 버튼
		man=new JRadioButton("남자");
		wo=new JRadioButton("여자");
		man.setBounds(100, 220, 70, 30);
		wo.setBounds(175, 220, 70, 30);
		add(man);
		add(wo);
		...
		// 우편번호
		post=new JTextField();
		post.setBounds(100, 255, 150, 30);
		add(post);
		
		// 우편번호 검색
		postFind=new JButton("우편번호 검색");
		postFind.setBounds(260, 255, 100, 30);
		add(postFind);
		
		// 주소
		addr1=new JTextField();
		addr1.setBounds(100, 290, 300, 30);
		add(addr1);
		
		// 상세주소
		addr2=new JTextField();
		addr2.setBounds(100, 325, 300, 30);
		add(addr2);
		
		// 전화
		phone=new JTextField();
		phone.setBounds(100, 360, 150, 30);
		add(phone);
		
		// 소개
		content=new JTextArea();
		JScrollPane js=new JScrollPane(content);
		js.setBounds(100, 395, 300, 100);
		add(js);
		...
		p.setBounds(40, 505, 370, 35); // 회원가입, 취소 버튼 크기 및 위치
		add(p);
